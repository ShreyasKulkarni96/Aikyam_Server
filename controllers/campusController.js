...
const getAllCampuses = asyncWrapper(async (req, res, next) => {
    const campusesCount = await CampusModel.count();
    ...
    successResp(res, { campusesCount, campuses }, 'Fetched successfully');
});
...
const deleteCampus = asyncWrapper(async (req, res, next) => {
    ...
    if (createdByUser !== "SUPER_ADMIN") {
        throw new AppError(403, 'You are unauthorized to delete a campus');
    }
