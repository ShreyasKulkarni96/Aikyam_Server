...
const updateSession = asyncWrapper(async (req, res, next) => {
    ...
    console.log(req)
    // VALIDATIONS
    const validatedSessionDetails = req.body;

    const session = await SessionModel.findByPk(req.body.id);
    if (!session) throw new AppError(400, `session with Id ${req.body.id} does not exist`);
