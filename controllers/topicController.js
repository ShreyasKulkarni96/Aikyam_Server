...
const updateTopic = asyncWrapper(async (req, res, next) => {
    ...
    const validatedTopic = req.body;

    // Find the Topic to be updated
    const topic = await TopicModel.findByPk(req.body.id);
    if (!topic) throw new AppError(400, `Topic with Id ${req.body.id} does not exist`);
