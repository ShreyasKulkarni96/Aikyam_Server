...
const updateCourse = asyncWrapper(async (req, res, next) => {
    ...
    const validatedCourse = req.body;
    // Find the course to be updated
    const course = await CourseModel.findByPk(validatedCourse.id);
    if (!course) throw new AppError(400, `Course with Id ${validatedCourse.id} does not exist`);
